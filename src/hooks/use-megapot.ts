...
import type { MegaPotData, UseMegaPotReturn } from "@/lib/interfaces";

export function useMegaPot(): UseMegaPotReturn {
  const [data, setData] = useState<MegaPotData | null>(null);
  ...
  const fetchMegaPot = useCallback(() => {
    setIsLoading(true);
    setError(null);
  ...
  const sendMegaPotRequest = () => {
    ...
    handleMegaPotResponse = (message: any) => {
      ...
      if (message.type === "error" && message.requestId === requestId) {
        ...
        if (message.code === "AUTH_REQUIRED") {
          ...
          setError(message.message || "Failed to fetch mega pot data");
        }

        setIsLoading(false);
        wsClient.off("getMegaPot_response", handleMegaPotResponse);
        wsClient.off("error", handleMegaPotResponse);
        return;
      }
      ...
      if (message.type === "getMegaPot_response") {
        ...
        if (message.requestId === requestId) {
          ...
          if (message.payload?.success) {
            const megaPotData: MegaPotData = message.payload.data;
            setData(megaPotData);
            setError(null);
          } else {
            setError(
              message.payload?.message || "Failed to fetch mega pot data"
            );
          }
          setIsLoading(false);
    ...
    wsClient.on("error", handleMegaPotResponse);
  ...
  useEffect(() => {
    const handleConnection = () => {
      ...
      if (isLoadingRef.current && !dataRef.current) {
        console.log("WebSocket connected, retrying mega pot request...");
        fetchMegaPot();
      }
    };
  ...
  const refetchMegaPot = useCallback(() => {
