...
function GameCard({
  ...
  return (
    <Card
      className={`relative text-center flex flex-col border-2 border-primary/20 bg-[#0e0e12] bg-opacity-80 hover:bg-opacity-100 hover:border-primary/40 transition-all duration-300 h-full overflow-hidden group ${
        disabled ? "opacity-50" : ""
      }`}
    >
      {bgImage && (
        <div className="absolute inset-0">
          <img
            ...
            alt=""
            className="w-full h-full object-cover opacity-10 transition-opacity"
            onError={(e) => {
              const target = e.target as HTMLImageElement;
              target.src = `/landing/card1.png?v=${Date.now()}`;
              target.classList.add("bg-gray-800");
            }}
            onLoad={(e) => {
              (e.target as HTMLImageElement).classList.remove("bg-gray-800");
            }}
          />
        </div>
      )}
      ...
      <CardContent className="flex-grow flex flex-col items-center justify-end z-10 p-4 pt-0">
        <Button
          asChild
          variant="default"
          className={`mt-auto bg-primary hover:bg-primary/90 text-white text-sm py-2 px-4 w-fit ${
